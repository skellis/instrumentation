...
There is a sleep time required for the translation stage to move. The time it takes is estimated by stagerange, 
maxtranslationTime, and maxinterval these values should be changed if a different translation stage is installed.
...
class FSRSScan2(module.Experiment):
    def __init__(self):
        ...
        prop.append({"label": "Wait for Move (s)", 'type': 'input', 'value': '1', "info": 'int'})        
        ...
        prop.append({"label": "Output Path", "type": "file", "value": "C:\Users\Saykally-EK-Beamtime\Desktop\Ellis\DATA\DefaultData", "info": "path"})
    ...
    def onStart(self, event=None):
        if self.running:
            ...
            self.Nsets = s_sets
            s_wait4move=int(self.getPropertyByLabel("Wait for Move (s)").getValue())
            self.wait4move=s_wait4move
            ...
            module.Experiment.start(self, ScanThread, type=s_type, ccd=s_ccd, axis=s_axis, shutter=s_shutter, frames=s_frames, points=s_points, sets=s_sets, reference=s_ref,wait4move=s_wait4move)
