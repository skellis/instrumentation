...
NewmarkStage.py is a module for controlling a 3 axis motion controll. 
As it is currently set up the axis AX are AY are translation stages and AZ is a rotation stage.
The code writes a scaling factor to the motion controller which converts the user units to milimeters of translation
We then use a definition mm2fs() to convert to femtoseconds of delay. Three velocity/acceleration profiles are allowed 
however only 10 mm/s is currently supported by FSRSScan2.py. Future work should involve writing a text file called 
"last_position.txt" so that T0 is can be recovered in the event that pyFSRS.py crashes.

This file belongs in /installed_modules/Devices/
..
   ...
   Copyright 2018 Scott Ellis <skellis@berkeley.edu>.
"""
# base class
...
class NewmarkStage(module.Axis):
    ...
    def __init__(self):
        ...
        self.steps2degrees=110000000.
        self.home = -3000.0                       #: homing position = most negative position allowed
        self.motors = ["AX","AY","AZ"]            #: note AZ is 
        #self.motors = ["AX"]
        self.speeds=["10","5","2","1","200"]
        ...
        prop.append({'label': 'Warning:', 'type': 'label', 'value': 'When pyFSRS freezes the t0 position will be lost. It is recommended that you mark your position in pencil on the stage.'})
    ...
    def connect(self, event=None):
        ...
        # set units to MM
        # set the acceleration and velocity in mm/s and mm/s^2 and move to -3000fs
        for m in self.motors:
            if m=="AX" or m=="AY":
                ...
                self.ser.write("%s;UU%f;VL%s;AC%s;MA%f;GO;" % (m, self.steps2degrees,self.getCurrentSpeed(),self.getCurrentSpeed(),0 * self.fs2mm))
        ...
        if self.ready:
            ...
            for m in self.motors:
                if m=="AX" or m=="AY":
                    ...
                    self.ser.write("%s;VL%s;AC%s;MA%f;GO;" % (m,self.getCurrentSpeed(),self.getCurrentSpeed(), 0 * self.fs2mm))
